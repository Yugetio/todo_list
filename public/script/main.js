(function(){
  ...
  function showList() {
    ...
    //clear
    document.querySelector('.create-item input[type=text]').value = '';
    document.querySelector('.create-item input[type=checkbox]').checked = false;
  }
  ...
  function setOnclick() {
    ...
    for (let i = 0; i < deleteBtn.length; i++) {
      deleteBtn[i].onclick = function (e) {
        fetch("/item", {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            id: data[e.target.parentNode.parentNode.rowIndex-1].id
          })
        })
        .then(res => {
    ...
    for (let i = 0; i < editBtn.length; i++) {
      editBtn[i].onclick = function (e) {
        ...
        const el = data[e.target.parentNode.parentNode.rowIndex-1];
